...
int JsonHelper::GetInt(char* key)
{
...
double JsonHelper::GetDouble(char* key)
{
   if (!_jsonData[key].isDouble())
   {
      return -1;
...
int JsonHelper::GetCString(char* key, char* buffer, int bufferSize)
{
   ...
   else if (bufferSize > static_cast<int>(strlen(data)))
   {
...
std::string JsonHelper::GetString(char* key)
{
   if (!_jsonData[key].isString())
   {
      return "";
...
bool JsonHelper::GetBool(char* key)
{
   if (!_jsonData[key].isBool())
   {
      return false;
...
JsonHelper* JsonHelper::GetJsonObject(char* key)
{
...
int JsonHelper::GetArraySize(char* key)
{
...
JsonHelper* JsonHelper::GetArray(char* key)
{
   if (!_jsonData[key].isArray())
   {
      return nullptr;
